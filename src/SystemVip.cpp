...
void SystemVip::LoadConfig() {
    ...
    loginMessage = sConfigMgr->GetOption<string>("SystemVip.LoginAnnounceMessage", "Player Vip: {} has logged in.");
...
string SystemVip::getInformationAdavantages() {
    ...
    text << "----------------------------------\n";
    if(loginAnnounce)
        text << "|TInterface/ICONS/Spell_unused2:15:15:-10:-5|t Announcement at login." << "\n";
    if (rateCustom) {
        text << "|TInterface/ICONS/Achievement_BG_KillXEnemies_GeneralsRoom:15:15:-10:-8|t Rate XP           x " << rateXp << "\n";
        text << "|TInterface/ICONS/Achievement_BG_overcome500disadvantage:15:15:-10:-8|t Rate professions  x " << professionRate << "\n";
    ...
    if(ghostMount)
        text << "|TInterface/ICONS/ability_vanish:15:15:-10::-8|t Speed ​​when being a ghost." << "\n";
    if (petEnable) {
        ...
        if(armorRep)
            text << "|TInterface/ICONS/INV_Hammer_20:15:15:-10::-8|t Repair Armor." << "\n";
        if(bankEnable)
            text << "|TInterface/ICONS/INV_Ingot_03:15:15:-10::-8|t Personal bank." << "\n";
        if(mailEnable)
            text << "|TInterface/ICONS/inv_letter_15:15:15:-10::-8|t Open mail." << "\n";
        if(buffsEnable)
        ...
        if(sicknessEnbale)
            text << "|TInterface/ICONS/spell_shadow_deathscream:15:15:-10::-8|t Eliminate sickness." << "\n";
        if(deserterEnable)
            text << "|TInterface/ICONS/ability_druid_cower:15:15:-10::-8|t Eliminate deserter." << "\n";
        if(resetInstance)
            text << "|TInterface/ICONS/Achievement_Dungeon_Icecrown_IcecrownEntrance:15:15:-10::-8|t Reset instancess." << "\n";
        if(saveTeleport)
            text << "|TInterface/ICONS/Spell_Holy_LightsGrace:15:15:-10::-8|t Save zones for teleport." << "\n";
    }
    return text.str();
}
...
void SystemVip::sendGossipInformation(Player* player, bool advantages) {
    ...
    if (isVip(player)) {
        ...
        text << "You don't have a VIP subscription available.\n";
        text << "Buy a subscription and enjoy all the benefits of being a VIP!\n";
    }

    if (advantages) {
        text << "Remember that when you buy VIP, you have benefits on all the characters in your account." << "\n";
...
string SystemVip::getLoginMessage(Player* player) {
    ...
    size_t pos = welcomeMessage.find("{}");
...
void SystemVip::addTeleportVip(Player* player, string name) {
    ...
    if (teleportMap.count(accountId) > 0) {
        if (teleportMap[accountId].size() == saveTeleportAmount) {
            ChatHandler(player->GetSession()).PSendSysMessage("You can't save any more Teleports!");
            return;
        }

        for (size_t i = 0; i < teleportMap[accountId].size(); i++) {
            if (teleportMap[accountId][i].name == teleport.name) {
                ChatHandler(player->GetSession()).PSendSysMessage("There is already a teleport with the same name!");
                return;
            }
        }
...
void SystemVip::delTeleportVip(Player* player, string name) {
    ...
    ChatHandler(player->GetSession()).PSendSysMessage("Wrong name.");
}
