...
public:
    ...
    void OnPlayerLogin(Player* player) override
    {
        ...
        if (sV->isVip(player))
            ChatHandler(player->GetSession()).PSendSysMessage("VIP subscription time available: |cff4CFF00{}|r", sV->getFormatedVipTime(player).c_str());
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        AddGossipItemFor(player, GOSSIP_ICON_TALK, "|TInterface/ICONS/INV_Misc_Coin_02:28:28:-15:0|t Vip Subscription.", 0, 1, "Do you want to subscribe to the VIP system for 7 days?\nPrice: " + to_string(sV->TokenAmount) + "\n " + sV->TokenIcon + " " +sV->getItemLink(sV->TokenEntry, player), 0, false);
        AddGossipItemFor(player, GOSSIP_ICON_TALK, "|TInterface/ICONS/INV_Misc_QuestionMark:28:28:-15:0|t Information.", 0, 2);
        if(sV->isVip(player))
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "|TInterface/ICONS/ability_hunter_beastcall:28:28:-15:0|t Get my VIP pet back.", 0, 4);
        ...
        switch (action)
        {
            case 1:
                if (player->HasItemCount(sV->TokenEntry, sV->TokenAmount))
                {
                    ...
                    ChatHandler(player->GetSession()).PSendSysMessage("VIP subscription time available: {}", sV->getFormatedVipTime(player).c_str());
                    OnGossipSelect(player, creature, 0, 2);
                }
                else
                {
                    ChatHandler(player->GetSession()).PSendSysMessage("You don't have enough Tokens.");
...
public:
    ...
    bool OnUse(Player* player, Item* /*item*/, SpellCastTargets const& /*targets*/)
    {
        ...
        {
            ChatHandler(player->GetSession()).PSendSysMessage("You can't use it in Arena!");
            return false;
        }
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if(sV->sicknessEnbale)
            AddGossipItemFor(player, 0, "|TInterface/ICONS/spell_shadow_deathscream:28:28:-15:0|t Remove sickness.", 0, 6);
        if(sV->deserterEnable)
            AddGossipItemFor(player, 0, "|TInterface/ICONS/ability_druid_cower:28:28:-15:0|t Remove deserter.", 0, 7);
        if(sV->resetInstance)
            AddGossipItemFor(player, 0, "|TInterface/ICONS/Achievement_Dungeon_Icecrown_IcecrownEntrance:28:28:-15:0|t Reset instancess.", 0, 9);
        if(sV->saveTeleport)
            AddGossipItemFor(player, 0, "|TInterface/ICONS/Spell_Holy_LightsGrace:28:28:-15:0|t My teleport.", 0, 10);
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action)
    {
        ...
        switch (action)
        {
            ...
            case 2:
                ...
                ChatHandler(player->GetSession()).PSendSysMessage("You repaired your armor.");
                OnGossipHello(player, creature);
                break;
            case 3:
            ...
            case 4:
                ...
                ChatHandler(player->GetSession()).PSendSysMessage("Buffs for you!");
                OnGossipHello(player, creature);
                break;
            case 5:
                if (player->IsInCombat())
            ...
            case 6:
                ...
                ChatHandler(player->GetSession()).PSendSysMessage("Your sickness was removed.");
                creature->CastSpell(player, 31726, true);
                OnGossipHello(player, creature);
                break;
            case 7:
                ...
                ChatHandler(player->GetSession()).PSendSysMessage("Your deserter mark has been removed.");
            ...
            case 10:
                // Sistema teleports
                AddGossipItemFor(player, 0, "|TInterface/GUILDBANKFRAME/UI-GuildBankFrame-NewTab:28:28:-15:0|t Add new.", 0, 1, "Name to save your coordinates.", 0, true);
                AddGossipItemFor(player, 0, "|TInterface/PAPERDOLLINFOFRAME/UI-GearManager-Undo:28:28:-15:0|t Eliminate.", 0, 2, "Name to delete.", 0, true);
